...
Points sourie_sur_un_point(Points sourie, Points p){
  if(sourie->x<p->x+10 && sourie->x>p->x-10){
    if(sourie->y<p->y+10 && sourie->y>p->y-10){
      return p;
    }
  }
  return NULL;
...
Points sur_un_point_poly(Points p_sourie,polygone p){
  Points p_select;
  if(!est_list_vide(p)){
    p_select=sourie_sur_un_point(p_sourie,renvoie_sommet_liste(p));
    if(p_select!=NULL){
      return p_select;
    }
    return sur_un_point_poly(p_sourie,liste_sans_premier(p));
  }
  return NULL;
}

Points sur_un_point(Points p_sourie,liste l_poly){
  Points p_select;
  if(!est_list_vide(l_poly)){
    p_select=sur_un_point_poly(p_sourie,renvoie_sommet_liste(l_poly));
    if(p_select!=NULL){
      return p_select;
    }
    return sur_un_point(p_sourie,liste_sans_premier(l_poly));
  }
  return NULL;
}
